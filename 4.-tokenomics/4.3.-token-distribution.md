...
Focusing on the data model explained in the [token utility](./4.2.-token-utility.md#data-model) section, the Treasury *(of 200M)* will be allocated to the Economy Engine, and the Liquidity *(of 200M)* will be first used to cover the initial **$GRYPH** rewards as a *gasless hot swap* in the platform.
